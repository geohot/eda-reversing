...
bool InstructionARM::initDataProcessing() {
  ...
  //R3 LSL 4
  {
    ...
    if (i->dpis.shift_imm != 0 || i->dpis.shift != 0) //this shift is real
    {
      ...
      right = new StatelessData((int) i->dpis.Rm, OPERATION_LSL
          + (i->dpis.shift), new StatelessData((Data) i->dpis.shift_imm));
  ...
  {
    mString << registersARM(i->dprs.Rm) << " " << shiftsARM(i->dprs.shift)
        << " " << registersARM(i->dprs.Rs);
    right = new StatelessData((int) i->dprs.Rm,
        OPERATION_LSL + (i->dprs.shift), new StatelessData((int) i->dprs.Rs));
  }
  ...
  if (mEncodingARM == ARM_LSM) {
    ...
    for (int a = 0; a < 16; a++) //the registers
    {
      if (reg & 0x1) {
        ...
        mAction.addChange(StatelessData(OPERATION_DEREF, offset),
            StatelessData(a));
        //print it
    ...
    if (i->lsm.W) //handle writeback
      mAction.addChange(StatelessData((int) i->generic.Rn), StatelessData(
          (int) i->generic.Rn, oper, new StatelessData((Data) rcount * 4)));
  } else {
    ...
    if (mEncodingARM == ARM_LSIO) {
      if (i->lsio.immed != 0) {
        ...
        StatelessData *g;
        if (i->lsio.U)
          g = new StatelessData((int) i->generic.Rn, OPERATION_ADD,
              new StatelessData((Data) i->lsio.immed));
        else
          g = new StatelessData((int) i->generic.Rn, OPERATION_SUB,
              new StatelessData((Data) i->lsio.immed));
        mAction.addChange(StatelessData((int) i->generic.Rd), StatelessData(
            OPERATION_DEREF, g));
      } else
        mAction.addChange(StatelessData((int) i->generic.Rd), StatelessData(
            OPERATION_DEREF, new StatelessData((int) i->generic.Rn)));
      ...
      if (i->lsro.shift_imm != 0 || i->lsro.shift != 0) {
        ...
        right = new StatelessData((int) i->lsro.Rm, OPERATION_LSL
            + (i->lsro.shift), new StatelessData((Data) i->lsro.shift_imm));
      } else
        right = new StatelessData((int) i->lsro.Rm);
      right = new StatelessData((int) i->generic.Rn, OPERATION_ADD, right);
      mAction.addChange(StatelessData((int) i->generic.Rd), StatelessData(
          OPERATION_DEREF, right));
    }
    mString.add("]", DT_SYMBOL);
  }
  return true;
...
bool InstructionARM::initBranches() {
  ...
  if (i->bbl.L)
    mString.add("L", DT_FLAG);
  mString << conditionsARM(i->generic.cond) << " ";
  //********************Data********************
  //  **First**
  int bl = ((i->bbl.offset & 0x800000) ? (0xFC000000) : 0) | (i->bbl.offset
      << 2);

  mString.add((Data)(bl + 8), DT_OFFSETADDRESS);

  if (bl < 0)
    mString << registersARM(REG_PC) << "-" << (0 - (Data) bl);
  else if (bl > 0)
    mString << registersARM(REG_PC) << "+" << (Data) bl;
  else
    mString << registersARM(REG_PC);

  mAction.addChange(StatelessData((int) REG_PC), StatelessData((int) REG_PC,
      OPERATION_ADD, new StatelessData((Data) bl)));
...
bool InstructionARM::initMiscellaneous() {
  ...
  if (i->mi.L) {
    ...
    if (i->mi.mb_c)
      mString.add("c", DT_SYMBOL);
    if (i->mi.mb_x)
      mString.add("x", DT_SYMBOL);
    if (i->mi.mb_s)
      mString.add("s", DT_SYMBOL);
    if (i->mi.mb_f)
      mString.add("f", DT_SYMBOL);
  }
