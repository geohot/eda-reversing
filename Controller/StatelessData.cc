...
//should really be const
{
  if (mDataType == DATATYPE_CONST)
    ...
    return (*r)[mRegister][changelistNumber];
  else if (mDataType == (DATATYPE_REG | DATATYPE_OPER)) {
    return evaluateOperation((*r)[mRegister][changelistNumber],
        mOperand->resolve(changelistNumber, r, m), mOperation);
...
Location StatelessData::resolveLocation(int changelistNumber, RegisterFile *r,
    Memory *m)
//add state
{
  if (mDataType == DATATYPE_CONST)
    return Location(mData); //memory address
  else if (mDataType == DATATYPE_REG)
    return Location(mRegister); //register
...
void Location::assign(int changelistNumber, Data value, RegisterFile *r,
    Memory *m) {
  switch (mDataType) {
