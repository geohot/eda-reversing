...
Changelist StatelessChangelist::resolve(int changelistNumber, RegisterFile *r,
    Memory *m)
//walk over the StatelessData and add state
{
  Changelist ret(changelistNumber);
  std::vector<std::pair<StatelessData, StatelessData> >::iterator walk =
      mInternalChangelist.begin();
  while (walk != mInternalChangelist.end()) {
    ret.addChange(walk->first.resolveLocation(changelistNumber, r, m),
        walk->second.resolve(changelistNumber, r, m));
    ++walk;
  }
  return ret;
}
...
bool StatelessChangelist::isRegisterAffected(int reg) {
  std::vector<std::pair<StatelessData, StatelessData> >::iterator walk =
      mInternalChangelist.begin();
  while (walk != mInternalChangelist.end()) {
    if (walk->first.mDataType == DATATYPE_REG && walk->first.mRegister == reg)
      return true;
    ++walk;
  }
  return false;
