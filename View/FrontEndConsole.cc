...
//should be min nonambiguous, w/e
  ...
  {
    mBank->lock(LOCKED_SERVER);
    mBank->mem()->consoleDump(strtoint(argv[1]),strtoint(argv[2]), strtoint(argv[3]));
    mBank->unlock(LOCKED_SERVER);
  }
  else if(argv[0]=="mw" && argv.size()==4) {
    //mw <addr> <data> <clnum>
    mBank->lock(LOCKED_SERVER);
    (*(mBank->mem()))[strtoint(argv[1])].set(strtoint(argv[3]), strtoint(argv[2]));
    mBank->unlock(LOCKED_SERVER);
  }
  else if(argv[0]=="segments")
  {
    ...
    mBank->lock(LOCKED_SERVER);
    std::map<Data,Instruction>::iterator i =
      mBank->mInstructionCache.find(strtoint(argv[1]));
